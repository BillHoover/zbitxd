...
void i2cbb_init(uint8_t pin_number_sda, uint8_t pin_number_scl) {
  ...
  delayTicks = 400; // Delay value empirically chosen to be twice the value that
                    // just start to cause I2C NACKs - N3SB
...
static int
read_SCL() { // Set SCL as input and return current level of line, 0 or 1
  pinMode(PIN_SCL, INPUT);
  return digitalRead(PIN_SCL);
}

static int
read_SDA() { // Set SDA as input and return current level of line, 0 or 1
...
static void i2c_delay() {
  volatile unsigned int
      index; // keeps compiler from optimizing out an empty for-loop
...
// This executes the SMBus “write byte” protocol, returning negative errno else
// zero on success.
...
// This executes the SMBus “read byte” protocol, returning negative errno else a
// data byte received from the device.
...
// read = 1, write = 0
// http://www.totalphase.com/support/articles/200349176-7-bit-8-bit-and-10-bit-I2C-Slave-Addressing
// This executes the SMBus “block write” protocol, returning negative errno else
// zero on success.
...
// This executes the SMBus “block read” protocol, returning negative errno else
// the number of data bytes in the slave's response.
int32_t i2cbb_read_i2c_block_data(uint8_t i2c_address, uint8_t command,
                                  uint8_t length, uint8_t *values) {
  uint8_t address = (i2c_address << 1) | 0;
  /*
          //static int i2c_write_byte(int send_start, int send_stop, uint8_t
    byte) if (i2c_write_byte(1, 0, address)){ i2c_stop_cond();
