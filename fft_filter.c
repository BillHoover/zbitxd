...
char wisdom_file_f[] =
    STATEDIR "/sbitx_wisdom_f.wis"; // Moved to default data directory - N3SB
...
int window_filter(int const L, int const M, complex float *const response,
                  float const beta) {
  ...
  fftwf_plan fwd_filter_plan = fftwf_plan_dft_1d(
      N, buffer, buffer, FFTW_FORWARD, WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
  fftwf_plan rev_filter_plan = fftwf_plan_dft_1d(
      N, buffer, buffer, FFTW_BACKWARD, WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
...
void filter_print(struct filter *f) {
  ...
  printf("#Filter windowed FIR frequency coefficients\n");
  for (int n = 0; n < f->N; n++) {
    printf("%d,%.17f,%.17f\n", n, crealf(f->fir_coeff[n]),
           cimagf(f->fir_coeff[n]));
  }
}
