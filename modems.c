...
/*
        ...
        There are three functions called to implement almost all the digital
   modes
        1. There is the modem_init() that is used to initialize all the
   different modems.
        2. The modem_poll() is called about 10 times a second to check if any
   transmit/receiver changeover is needed, etc.
        3. On receive, each time a block of samples is received, modem_rx() is
   called and it despatches the block of samples to the currently selected
   modem. The demodulators call write_console() to call the routines to display
   the decoded text.
        4. During transmit, modem_next_sample() is repeatedly called by the sdr
   to accumulate samples. In turn the sample generation routines call
   get_tx_data_byte() to read the next text/ascii byte to encode.
...
int fldigi_call_i(char *action, int param, char *result) {
  ...
  /*	timeout.tv_sec = 0;
          ...
          setsockopt(fldigi_socket, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof
     timeout); setsockopt(fldigi_socket, SOL_SOCKET, SO_SNDTIMEO, &timeout,
     sizeof timeout);
  */
  *result = 0; // start with a null string so it is returned if nothing is read

  if (connect(fldigi_socket, (struct sockaddr *)&serverAddr,
              sizeof(serverAddr)) < 0) {
...
// each mode has its peculiarities, like the ft8 will start only on 15th second
// boundary psk31 will transmit a few spaces after the last character, etc.
