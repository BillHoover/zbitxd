...
static void get_console(struct mg_connection *c) {
  ...
  if (!n)
    return;
  // char first20 [21]; strncpy(first20, buff, 20); first20[20] = 0;
...
static void do_login(struct mg_connection *c, char *key) {
  ...
  hd_createGridList(); // llh: make the list up to date at the beginning of a
                       // session
...
static int16_t remote_samples[10000]; // the max samples are set by the queue
                                      // lenght in modems.c
...
static void web_despatcher(struct mg_connection *c, struct mg_ws_message *wm) {
  ...
  if (field == NULL || cookie == NULL) {
    ...
    c->is_draining = 1;
  } else if (strlen(field) > 100 || strlen(field) < 2 || strlen(cookie) > 40 ||
             strlen(cookie) < 4) {
...
void webserver_stop() { mg_mgr_free(&mgr); }
