/*
        ...
        The cw_get_sample() is also called from the thread that does DSP, so
   stalling it is dangerous.
        ...
        the key_poll stores the values read from the ISR of the PTT(DOT) and
   DASH gpio lines

        the cw_read_key() routine returns the next dash/dot/space/, etc to be
   sent the word 'symbol' is used to denote a dot, dash, a gaps that are dot,
   dash or word long. These are defined in sdr.h (they shouldn't be)
        ...
        3. We maintained a running average of the highs and the lows
   (corresponding to the signal peak and the noise floor). These are updated in
   a moving average as high_level and threshold elements in
   cw_rx_update_levels() function.

        4. In cw_rx_process (), we threshold the signal magnitude to generate
        'mark' and 'space'. Each of them in placed into a string of struct
   symbol. We maintain a track of the magnitude, time (in terms of ticks).
        ...
        7. The match_letter uses the symbols in terms of their magnitude,
   duration to first-fit a pattern from the morse_rx_table. This table should
   ideally be read from a text file. It could, in the future also contain
   callsign database. This function needs to be worked on to work
   probabilitisically with best match where the magnitude of the signal is
   marginally across the threshold between the signal and the noisefloor.
...
float cw_tx_get_sample() {
  ...
  case CW_DOT_DELAY:
    ...
    if (cw_mode == CW_IAMBICB) {
      if (cw_next_symbol == CW_IDLE && cw_last_symbol == CW_DOT &&
          (symbol_now & CW_DASH)) {
        cw_next_symbol = CW_DASH;
      }
      if (cw_next_symbol == CW_IDLE && cw_last_symbol == CW_DASH &&
          (symbol_now & CW_DOT)) {
        cw_next_symbol = CW_DASH;
      }
    }
...
static void cw_rx_update_levels(struct cw_decoder *p) {
  ...
  if (p->high_level < p->magnitude)
    ...
    p->high_level =
        (p->magnitude + ((HIGH_DECAY - 1) * p->high_level)) / HIGH_DECAY;
  ...
  if (p->magnitude < (p->high_level * 4) / 10) {
    ...
    if (p->magnitude < 100)
      p->magnitude = 100;
    p->noise_floor =
        (p->magnitude + ((NOISE_DECAY - 1) * p->noise_floor)) / NOISE_DECAY;
...
void cw_poll(int bytes_available, int tx_is_on) {
  ...
  if (!tx_is_on &&
      (cw_bytes_available || key_poll() || (symbol_next && *symbol_next)) > 0) {
    tx_on(TX_SOFT);
    millis_now = millis();
