...
int logbook_count_dup(const char *callsign, int last_seconds) {
  ...
  sprintf(date_str, "%04d-%02d-%02d", tmp->tm_year + 1900, tmp->tm_mon + 1,
          tmp->tm_mday);
...
bool logbook_caller_exists(char *id) {
  sqlite3_stmt *stmt;
  char *statement =
      "SELECT EXISTS(SELECT 1 FROM logbook WHERE callsign_recv=?)";
  int res = sqlite3_prepare_v2(db, statement, -1, &stmt, NULL);
  if (res != SQLITE_OK)
    return false;
  bool exists = false;
...
void message_add(char *mode, unsigned int frequency, int outgoing,
  ...
  int date_utc = ((tmp->tm_year + 1900) * 10000) + ((tmp->tm_mon + 1) * 100) +
                 (tmp->tm_mday);
...
void logbook_add(char *contact_callsign, char *rst_sent, char *exchange_sent,
  ...
  sprintf(statement,
          ...
          log_freq, mode, date_str, time_str, mycallsign, rst_sent,
          exchange_sent, contact_callsign, rst_recv, exchange_recv);
...
const static char *adif_names[] = {
    "ID",         "MODE",     "FREQ",       "QSO_DATE", "TIME_ON",
    "OPERATOR",   "RST_SENT", "STX_String", "CALL",     "RST_RCVD",
    "SRX_String", "STX",      "COMMENTS"};

struct band_name {
  char *name;
  int from, to;
} bands[] = {
    {"160M", 1800, 2000},  {"80M", 3500, 4000},   {"60M", 5000, 5500},
    {"40M", 7000, 7300},   {"30M", 10000, 10150}, {"20M", 14000, 14350},
    {"17M", 18000, 18200}, {"15M", 21000, 21450}, {"12M", 24800, 25000},
    {"10M", 28000, 29700},
};
...
int export_adif(char *path, char *start_date, char *end_date) {
  ...
  // add to the bottom of the logbook
  sprintf(statement,
          "select * from logbook where (qso_date >= '%s' AND  qso_date <= "
          "'%s')  ORDER BY id DESC;",
          start_date, end_date);
