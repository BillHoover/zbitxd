...
// Set the DEBUG define to 2 to compile in detailed error reporting debugging
// messages.
#define DEBUG 0
...
http://alsamodular.sourceforge.net/alsa_programming_howto.html Next thing to try
is http://www.saunalahti.fi/~s7l/blog/2005/08/21/Full%20Duplex%20ALSA
  ...
  At two periods in the buffer, the buffer has to be 16K
        ...
        To simply the work, we are picking up some settings for the Wolfson
codec as it connects to a raspberry pi. These values are interdependent and they
will work out of the box. It takes the guess work out of configuring the
Raspberry Pi with Wolfson codec.
*/
...
static snd_pcm_stream_t play_stream = SND_PCM_STREAM_PLAYBACK; // playback
                                                               // stream
static snd_pcm_stream_t capture_stream =
    SND_PCM_STREAM_CAPTURE; // playback stream
...
#define LOOPBACK_LEVEL_DIVISOR                                                 \
  8 // Constant used to reduce audio level to the loopback channel (FLDIGI)
...
// Note: Error messages appear when the sbitx program is started from the
// command line
...
The sequence of the alsa functions must be maintained for this to work
consistently
...
int sound_start_play(char *device) {
...
#endif
  int e = snd_pcm_open(&pcm_play_handle, device, play_stream,
                       0); // was SND_PCM_NONBLOCK
  ...
  e = snd_pcm_hw_params_set_access(pcm_play_handle, hwparams,
  ...
  /* Set sample format */
  e = snd_pcm_hw_params_set_format(pcm_play_handle, hwparams,
  ...
  /*
  // This function call and the next have been replaced by the
  snd_pcm_hw_params_set_period_size_near() function call - N3SB December 2023
          // frame = bytes_per_sample x n_channel
          // period = frames transfered at a time (160 for voip, etc.)
          // we use two periods per buffer.
          if ((e = snd_pcm_hw_params_set_periods(pcm_play_handle, hwparams,
  n_periods_per_buffer, 0)) < 0) { fprintf(stderr, "*Error setting playback
  periods.\n"); return(-1);
          }
  */
  ...
  snd_pcm_uframes_t n_frames =
      (buff_size * n_periods_per_buffer) / 8 * 4; // A Larger buffer - N3SB Hack
#if DEBUG > 0
  printf("trying for buffer size of %ld\n", n_frames);
#endif
  /*
  // This function call and the previous have been replaced by the
  snd_pcm_hw_params_set_period_size_near() function call - N3SB December 2023 e
  = snd_pcm_hw_params_set_buffer_size_near(pcm_play_handle, hwparams,
  &n_frames); if (e < 0) { fprintf(stderr, "*Error setting playback
  buffersize.\n"); return(-1);
          }
  */
  // This function call replaces the two function calls above - N3SB December
  // 2023
  e = snd_pcm_hw_params_set_period_size_near(pcm_play_handle, hwparams,
                                             &n_frames, 0);
  if (e < 0) {
...
int sound_start_loopback_capture(char *device) {
  ...
  e = snd_pcm_hw_params_set_access(loopback_capture_handle, hloop_params,
                                   SND_PCM_ACCESS_RW_INTERLEAVED);
  if (e < 0) {
  ...
  /* Set sample format */
  e = snd_pcm_hw_params_set_format(loopback_capture_handle, hloop_params,
                                   SND_PCM_FORMAT_S32_LE);
  if (e < 0) {
  ...
  e = snd_pcm_hw_params_set_rate(loopback_capture_handle, hloop_params,
                                 exact_rate, 0);
  if (e < 0) {
  ...
  /* Set number of channels */
  if ((e = snd_pcm_hw_params_set_channels(loopback_capture_handle, hloop_params,
                                          2)) < 0) {
  ...
  /*
          ...
          if ((e = snd_pcm_hw_params_set_periods(loopback_capture_handle,
     hloop_params, n_periods_per_buffer, 0)) < 0) { fprintf(stderr, "*Error
     setting loopback capture periods.\n"); return(-1);
          }

          // the buffer size is each periodsize x n_periods
          snd_pcm_uframes_t  n_frames= (buff_size  * n_periods_per_buffer) / 8;
          // printf("trying for buffer size of %ld\n", n_frames);
          e = snd_pcm_hw_params_set_buffer_size_near(loopback_capture_handle,
     hloop_params, &n_frames); if (e < 0) { fprintf(stderr, "*Error setting
     loopback capture buffersize.\n"); return(-1);
          }
  */
  // This function call and the previous have been replaced by the
  // snd_pcm_hw_params_set_period_size_near() function call - N3SB December 2023
  snd_pcm_uframes_t n_frames = (buff_size * n_periods_per_buffer) / 8;
  e = snd_pcm_hw_params_set_period_size_near(loopback_capture_handle,
                                             hloop_params, &n_frames, 0);
  if (e < 0) {
  ...
  if ((e = snd_pcm_sw_params_current(loopback_capture_handle, sloop_params)) <
      0) {
  ...
  if ((e = snd_pcm_sw_params_set_start_threshold(loopback_capture_handle,
                                                 sloop_params, 15)) < 0) {
    fprintf(stderr, "Unable to set threshold mode for loopback capture\n");
  }

  if ((e = snd_pcm_sw_params_set_stop_threshold(loopback_capture_handle,
                                                sloop_params, 1)) < 0) {
...
This ensures that the blocks are returned in perfect timing with the codec's
clock Once you process these captured samples and send them to the playback
device, you just wait for the next block to arrive
*/
...
int sound_start_capture(char *device) {
  ...
  e = snd_pcm_hw_params_set_access(pcm_capture_handle, hwparams,
                                   SND_PCM_ACCESS_RW_INTERLEAVED);
  if (e < 0) {
  ...
  /* Set sample format */
  e = snd_pcm_hw_params_set_format(pcm_capture_handle, hwparams,
                                   SND_PCM_FORMAT_S32_LE);
  if (e < 0) {
  ...
  /* Set number of channels */
  if ((e = snd_pcm_hw_params_set_channels(pcm_capture_handle, hwparams, 2)) <
      0) {
  ...
  /*
          // Set number of periods. Periods used to be called fragments.
          if ((e = snd_pcm_hw_params_set_periods(pcm_capture_handle, hwparams,
     n_periods_per_buffer, 0)) < 0) { fprintf(stderr, "*Error setting capture
     periods.\n"); return(-1);
          }
          ...
          snd_pcm_uframes_t  n_frames= (buff_size  * n_periods_per_buffer)/ 8;
          //printf("trying for buffer size of %ld\n", n_frames);
          e = snd_pcm_hw_params_set_buffer_size_near(pcm_capture_handle,
     hwparams, &n_frames); if (e < 0) { fprintf(stderr, "*Error setting PCM
     capture buffersize.\n"); return(-1);
          }
  */
  snd_pcm_uframes_t n_frames = (buff_size * n_periods_per_buffer) / 8;
  // This function call replaces the two function calls above - N3SB December
  // 2023
  e = snd_pcm_hw_params_set_period_size_near(pcm_capture_handle, hwparams,
                                             &n_frames, 0);
  if (e < 0) {
...
int sound_start_loopback_play(char *device) {
...
#endif
  int e = snd_pcm_open(&loopback_play_handle, device, play_stream,
                       0); // was SND_PCM_NONBLOCK
  ...
  e = snd_pcm_hw_params_set_access(loopback_play_handle, hwparams,
                                   SND_PCM_ACCESS_RW_INTERLEAVED);
  if (e < 0) {
  ...
  /* Set sample format */
  e = snd_pcm_hw_params_set_format(loopback_play_handle, hwparams,
                                   SND_PCM_FORMAT_S32_LE);
  if (e < 0) {
  ...
  /* Set number of channels */
  if ((e = snd_pcm_hw_params_set_channels(loopback_play_handle, hwparams, 2)) <
      0) {
    fprintf(stderr, "*Error setting playback channels.\n");
    return (-1);
  }
  ...
  /*
          ...
          // period = frames transfered at a time (160 for voip, etc.)
          // we use two periods per buffer.
          if ((e = snd_pcm_hw_params_set_periods(loopback_play_handle, hwparams,
     8, 0)) < 0) { fprintf(stderr, "*Error setting playback periods.\n");
                  return(-1);
          }
          ...
          e = snd_pcm_hw_params_set_buffer_size_near(loopback_play_handle,
     hwparams, &n_frames); if (e < 0) { fprintf(stderr, "*Error setting loopback
     playback buffersize.\n"); return(-1);
          }
  */
  // This function call replaces the two function calls above - N3SB December
  ...
  // lets pump it up to see if we can reduce the dropped frames
  n_frames *= 4;
  e = snd_pcm_hw_params_set_period_size_near(loopback_play_handle, hwparams,
                                             &n_frames, 0);
  if (e < 0) {
...
void sound_process2(int32_t *input_i, int32_t *input_q, int32_t *output_i,
                    int32_t *output_q, int n_samples) {
...
unsigned long sbitx_millis() { return sound_millis; }
...
int sound_loop() {
  int32_t *line_in, *line_out, *data_in, *data_out, *input_i, *output_i,
      *input_q, *output_q;
  ...
  */
  ...
  while (sound_thread_continue) {
    ...
    pcm_read_new_time =
        1000000 * tv.tv_sec + tv.tv_usec; // Store time in microseconds
    delta_time = pcm_read_new_time - pcm_read_old_time;
    if ((delta_time > 11667) ||
        (delta_time < 9667)) // Loop should iterate every 10667 microseconds
    {
#if DEBUG > 1
    ...
    while ((pcmreturn = snd_pcm_readi(pcm_capture_handle, data_in, frames)) <
           0) {
    ...
    sound_millis =
        (gettime_now.tv_sec * 1000) + (gettime_now.tv_nsec / 1000000);
    ...
    while (framesize > 0) {
      ...
      do {
        pcmreturn =
            snd_pcm_writei(pcm_play_handle, data_out + offset, framesize);
      } while (pcmreturn == -11);

      if ((pcmreturn > 0) && (pcmreturn < 1024)) {
#if DEBUG > 0
        printf(
            "#### Partial Write ####"); // would signify that the playback
                                        // channel didn't accept all the samples
#endif
      }
      // if((pcmreturn < 0) && (pcmreturn != -11))	// also ignore
      ...
      if (pcmreturn < 0) {
        ...
        printf("Loop Counter: %d, Play PCM Write Error %d: %s  count = %d\n",
               loop_counter, pcmreturn, snd_strerror(pcmreturn),
               pcm_play_write_error++);
#endif
        if (pcmreturn == -EPIPE) {
#if DEBUG > 0
          printf("Samples Read: %d, Samples Written: %d, delta: %d, available "
                 "%d\n",
      ...
      if (pcmreturn >= 0) {
        // Calculate remaining number of samples to be sent and new position in
        // sample array. If all the samples were processed by the snd_pcm_writei
        // function then framesize will be zero and the while() loop will end.
        if (pswitch == 1) {
#if DEBUG > 0
          printf("%d ", pcmreturn);
#endif
    ...
    while (ii < ret_card) {
      line_out[jj++] =
          output_i[ii] / LOOPBACK_LEVEL_DIVISOR; // Left Channel. Reduce audio
                                                 // level to FLDIGI a bit
      line_out[jj++] = output_i[ii] /
                       LOOPBACK_LEVEL_DIVISOR; // Right Channel. Note: FLDIGI
                                               // does not use the this channel.

      // The right channel can be used to output other integer values such as
      // AGC, for capture by an application such as audacity.
      ii += 2; // Skip a pair of samples to account for the 96K sample to 48K
               // sample rate change.
    }

    // This is the new pcm loopback write routine
    framesize = (ret_card + 1) / 2; // only writing half the number of samples
                                    // because of the slower channel rate
    offset = 0;
    ...
    while (framesize > 0) {
      ...
      do {
        pcmreturn =
            snd_pcm_writei(loopback_play_handle, line_out + offset, framesize);
      } while (pcmreturn == -11);

      // if((pcmreturn < 0) && (pcmreturn != -11))	// also ignore
      // "temporarily unavailable" errors
      if (pcmreturn <
          0) { // Handle an error condition from the snd_pcm_writei function
#if DEBUG > 0
        printf("Loopback PCM Write %d bytes Error %d: %s  count = %d\n",
               framesize, pcmreturn, snd_strerror(pcmreturn),
               pcm_loopback_write_error++);
#endif

#if DEBUG < 2
        snd_pcm_recover(loopback_play_handle, pcmreturn,
                        1); // Does not provide detailed error message
#else
        snd_pcm_recover(loopback_play_handle, pcmreturn,
                        0); // Provides detailed error message
#endif
      }
...
int loopback_loop() {
  ...
  while (sound_thread_continue) {
    ...
    while ((pcmreturn = snd_pcm_readi(loopback_capture_handle, data_in,
                                      frames / 2)) < 0) {
      snd_pcm_prepare(loopback_capture_handle);
      // putchar('=');
    }
...
int sound_thread_start(char *device) {
  ...
  sleep(1);
  pthread_create(&loopback_thread, NULL, loopback_thread_function,
                 (void *)device);
}

void sound_thread_stop() { sound_thread_continue = 0; }
...
/*
void sound_process(int32_t *input_i, int32_t *input_q, int32_t *output_i,
int32_t *output_q, int n_samples){
