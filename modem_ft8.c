...
/// The pulse is theoretically infinitely long, however, here it's truncated at
/// 3 times the symbol length. This means the pulse array has to have space for
/// 3*n_spsym elements.
...
/// @param[in] symbol_bt Symbol smoothing filter bandwidth (2 for FT8, 1 for
/// FT4)
/// @param[in] symbol_period Symbol period (duration), seconds
/// @param[in] signal_rate Sample rate of synthesized signal, Hertz
/// @param[out] signal Output array of signal waveform samples (should have
/// space for n_sym*n_spsym samples)
///
static void synth_gfsk(const uint8_t *symbols, int n_sym, float f0,
                       float symbol_bt, float symbol_period, int signal_rate,
                       float *signal) {
...
int sbitx_ft8_encode(char *message, int32_t freq, float *signal, bool is_ft4) {
  ...
  int num_samples = (int)(0.5f + num_tones * symbol_period *
                                     sample_rate); // samples in the data signal
  int num_silence = (slot_time * sample_rate - num_samples) /
                    2; // Silence  to make 15 seconds
  int num_total_samples =
      num_silence + num_samples + num_silence; // total Number samples
...
static int sbitx_ft8_decode(float *signal, int num_samples, bool is_ft8) {
  ...
  monitor_t mon;
  monitor_config_t mon_cfg = {.f_min = 100,
                              .f_max = 3000,
  ...
  // Process the waveform data frame by frame - you could have a live loop here
  // with data from an audio device
  ...
  int num_candidates =
      ft8_find_sync(&mon.wf, kMax_candidates, candidate_list, kMin_score);
  ...
  for (int idx = 0; idx < num_candidates; ++idx) {
    ...
    if (found_empty_slot) {
      ...
      // message_add(char *mode, unsigned int frequency, int outgoing, char
      // *message);
...
static void ft8_start_tx(int offset_seconds) {
  ...
  sprintf(buff, "%02d%02d%02d  TX +00 %04d ~  %s\n", t->tm_hour, t->tm_min,
          t->tm_sec, ft8_pitch, ft8_tx_text);
  write_console(FONT_FT8_TX, buff);
  message_add("FT8", ft8_pitch, 1, ft8_tx_text);

  ft8_tx_nsamples =
      sbitx_ft8_encode(ft8_tx_text, ft8_pitch, ft8_tx_buff, false);
...
void *ft8_thread_function(void *ptr) {
  FILE *pf;
  char buff[1000], mycallsign_upper[20]; // there are many ways to crash sbitx,
                                         // bufferoverflow of callsigns is 1
...
void ft8_poll(int seconds, int tx_is_on) {
  ...
  if ((ft8_tx1st == 1 &&
      ...
      (ft8_tx1st == 0 &&
       ((seconds >= 15 && seconds < 30) || (seconds >= 45 && seconds < 59)))) {
    tx_on(TX_SOFT);
...
void ft8_on_start_qso(char *message) {
  ...
  // whoa, someone cold called us
    ...
    if (isalpha(m3[0]) && isalpha(m3[1]) &&
        strncmp(m3, "RR", 2) != 0) { // R- RR are not EXCH
      field_set("EXCH", m3);
    ...
    field_set("SENT", signal_strength);
    sprintf(reply_message, "%s %s %s", call, mycall,
            mygrid); // signal_strength);
  }
  field_set("NR", mygrid);
...
void ft8_process(char *message, int operation) {
  ...
  // see if you are on auto responder, the logger is empty and we are the called
  // party
  ...
  // this is a signal report, at times, other call can just send their sig
  // report
  if (m3[0] == '-' || (m3[0] == 'R' && m3[1] == '-') || m3[0] == '+' ||
      (m3[0] == 'R' && m3[1] == '+')) {
