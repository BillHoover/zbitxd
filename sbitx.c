...
// Options for WISDOM_MODE from least to most rigorous are FFTW_ESTIMATE,
// FFTW_MEASURE, FFTW_PATIENT, and FFTW_EXHAUSTIVE The FFTW_ESTIMATE mode seems
// to make completely incorrect Wisdom plan choices sometimes, and is not
// recommended. Wisdom plans found in an existing Wisdom file will negate the
// need for time consuming Wisdom plan calculations if the Wisdom plans in the
// file were generated at the same or more rigorous level.
#define WISDOM_MODE FFTW_EXHAUSTIVE
#define PLANTIME                                                               \
  -1 // spend no more than plantime seconds finding the best FFT algorithm. -1
     // turns the platime cap off.
char wisdom_file[] =
    STATEDIR "/sbitx_wisdom.wis"; // Moved to default data directory - N3SB

fftw_complex *
    fft_out; // holds the incoming samples in freq domain (for rx as well as tx)
fftw_complex
    *fft_in; // holds the incoming samples in time domain (for rx as well as tx)
fftw_complex
    *fft_m; // holds previous samples for overlap and discard convolution
...
struct power_settings band_power[] = {
    {3500000, 4000000, 37, 0.002},    {5200000, 5800009, 40, 0.0015},
    {7000000, 7300009, 40, 0.0015},   {10000000, 10200000, 35, 0.0019},
    {14000000, 14300000, 35, 0.0025}, {18000000, 18200000, 20, 0.0023},
    {21000000, 21450000, 20, 0.003},  {24800000, 25000000, 20, 0.0034},
...
void fft_init() {
  ...
  plan_fwd = fftw_plan_dft_1d(MAX_BINS, fft_in, fft_out, FFTW_FORWARD,
                              WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
  plan_spectrum = fftw_plan_dft_1d(MAX_BINS, fft_in, fft_spectrum, FFTW_FORWARD,
                                   WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
...
#define SCALING_TRIM                                                           \
  200.0 // Use this to tune your meter response 2.7 worked at 51% and my
        // inverted L
// S-Meter test W2JON
int calculate_s_meter() {
  ...
  for (int i = 0; i < MAX_BINS / 2; i++) {
    double magnitude = cabs(
        rx_list->fft_time[i]); // Magnitude of complex FFT output in time domain
  ...
  s_meter_db +=
      gain_scaling_factor *
      SCALING_TRIM; // Adjust calcs dynamically based on rx_gain * SCALING_TRIM

  // Calculate S-units and additional dB
  int s_units = (int)(s_meter_db / 6.0); // Each S-unit corresponds to 6 dB
  int additional_db =
      (int)(s_meter_db - (s_units * 6)); // Remaining 'dB' above S9
...
void set_rx1(int frequency) {
  static int last_frequency; // Holds the last frequency set - used by the Auto
                             // IF Gain algorithm
...
In this demo, we read and equivalent block from the file instead of processing
from the input I and Q signals.
*/
...
void tx_init(int frequency, short mode, int bpf_low, int bpf_high) {
  ...
  //	filter_tune(tx_filter, (1.0 * bpf_low)/96000.0, (1.0 * bpf_high)/96000.0
  //, 5);
}
...
struct rx *add_tx(int frequency, short mode, int bpf_low, int bpf_high) {
  ...
  r->plan_rev =
      fftw_plan_dft_1d(MAX_BINS, r->fft_freq, r->fft_time, FFTW_BACKWARD,
                       WISDOM_MODE); // Was FFTW_ESTIMATE N3SB
  fftw_export_wisdom_to_filename(wisdom_file);
  ...
  filter_tune(r->filter, (1.0 * bpf_low) / 96000.0, (1.0 * bpf_high) / 96000.0,
              5);
...
double agc2(struct rx *r) {
  ...
  // printf("%d:s meter: %d %d %d \n", count++, (int)r->agc_gain,
  // (int)r->signal_strength, r->agc_loop);
  return 100000000000 / r->agc_gain;
}

void my_fftw_execute(fftw_plan f) { fftw_execute(f); }
...
void rx_linear(int32_t *input_rx, int32_t *input_mic, int32_t *output_speaker,
  ...
  if (rx_list->output == 0) {
    ...
    // push the samples to the remote audio queue, decimated to 16000
    // samples/sec
...
void tx_process(int32_t *input_rx, int32_t *input_mic, int32_t *output_speaker,
                int32_t *output_tx, int n_samples) {
  int i;
  ...
  if (mute_count &&
      (r->mode == MODE_USB || r->mode == MODE_LSB || r->mode == MODE_AM)) {
  ...
  for (i = MAX_BINS / 2; i < MAX_BINS; i++) {
    if (r->mode == MODE_2TONE)
      i_sample =
          (1.0 * (vfo_read(&tone_a) + vfo_read(&tone_b))) / 50000000000.0;
...
void set_rx_filter() {
  ...
  if (rx_list->mode == MODE_AM) {
    filter_tune(rx_list->filter, (1.0 * (24000 - rx_list->high_hz)) / 96000.0,
                (1.0 * (24000 + rx_list->high_hz)) / 96000.0, 5);
  } else if (rx_list->mode == MODE_LSB || rx_list->mode == MODE_CWR)
    filter_tune(rx_list->filter, (1.0 * -rx_list->high_hz) / 96000.0,
                (1.0 * -rx_list->low_hz) / 96000.0, 5);
  else
    filter_tune(rx_list->filter, (1.0 * rx_list->low_hz) / 96000.0,
                (1.0 * rx_list->high_hz) / 96000.0, 5);
}
...
void signal_handler(int signum) { digitalWrite(TX_LINE, LOW); }
...
void sdr_request(char *request, char *response) {
  ...
  if (!strcmp(cmd, "stat:tx")) {
    ...
    if (rx_list->mode == MODE_AM) {
      filter_tune(tx_list->filter, (1.0 * 20000) / 96000.0,
                  (1.0 * 30000) / 96000.0, 5);
      filter_tune(tx_filter, (1.0 * 20000) / 96000.0, (1.0 * 30000) / 96000.0,
                  5);
    } else if (rx_list->mode == MODE_LSB || rx_list->mode == MODE_CWR) {
      filter_tune(tx_list->filter, (1.0 * -3000) / 96000.0,
                  (1.0 * -300) / 96000.0, 5);
      filter_tune(tx_filter, (1.0 * -3000) / 96000.0, (1.0 * -300) / 96000.0,
                  5);
    } else {
      filter_tune(tx_list->filter, (1.0 * 300) / 96000.0,
                  (1.0 * 3000) / 96000.0, 5);
